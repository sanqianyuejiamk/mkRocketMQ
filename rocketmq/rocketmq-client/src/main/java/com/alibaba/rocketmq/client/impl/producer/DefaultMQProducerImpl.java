...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DefaultMQProducerImpl.class);
    ...
    public void createTopic(String key, String newTopic, int readQueueNum, int writeQueueNum, int topicSysFlag)
            throws MQClientException {
        this.makeSureStateOK();
        Validators.checkTopic(newTopic);

        this.mQClientFactory.getMQAdminImpl().createTopic(key, newTopic, readQueueNum, writeQueueNum, topicSysFlag);
    }
    ...
    /**
     * 发送消息
     * 1.消息发送超时时间，maxTimeout=4s;
     * 2.消息发送失败重试次数，timesTotal=3(次)；
     *
     * @param msg
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal && (endTimestamp - beginTimestamp) < maxTimeout; times++) {
                ...
                if (tmpmq != null) {
                    ...
                    try {
                        //logger.info("send message toipic = " + mq.getTopic() + " , broker = " + mq.getBrokerName() + " , queueId = " + mq.getQueueId() + " communicationMode = "+communicationMode+" , content = " + new String(msg.getBody()));
    ...
    private SendResult sendKernelImpl(final Message msg,//
        ...
        if (brokerAddr != null) {
            return sendKernelImpl(msg, mq, communicationMode, sendCallback, timeout, brokerAddr);
        }
     ...
     * @throws InterruptedException
     */
    private SendResult sendKernelImpl(final Message msg, final MessageQueue mq, final CommunicationMode communicationMode, final SendCallback sendCallback, final long timeout, String brokerAddr)
