...
public class MQClientAPIImpl {
    ...
    public MQClientAPIImpl(final NettyClientConfig nettyClientConfig,
        ...
        this.remotingClient.registerProcessor(RequestCode.UPDATE_TOPIC_LEVEL,
                this.clientRemotingProcessor, null);
     ...
     * @param brokerName
     * @param msg
     * @param requestHeader     request请求数据
     * @param timeoutMillis     超时时间，默认4s
    ...
    public SendResult sendMessage(//
        ...
        logger.info("communicationMode = " + communicationMode + " , brokerAddr = " + brokerAddr + " , topic = " + msg.getTopic());
        switch (communicationMode) {
            case ONEWAY:
                this.remotingClient.invokeOneway(brokerAddr, request, timeoutMillis);
                return null;
            case ASYNC:
                this.sendMessageAsync(brokerAddr, brokerName, msg, timeoutMillis, request, sendCallback);
                return null;
            case SYNC:
                return this.sendMessageSync(brokerAddr, brokerName, msg, timeoutMillis, request);
            default:
                assert false;
                break;
        }
    ...
    private SendResult sendMessageSync(//
        ...
        logger.info("remotingClient sendMessageSync , addr = " + addr);
        RemotingCommand response = this.remotingClient.invokeSync(addr, request, timeoutMillis);
        assert response != null;
    ...
    private SendResult processSendResponse(//
        ...
        switch (response.getCode()) {
            ...
            case ResponseCode.SUCCESS: {
                ...
                logger.info(String.format("--------------- send message success! , processSendResponse topic = %s , broker = %s , msgId = %s , queue = %s , offset = %s", messageQueue.getTopic(), messageQueue.getBrokerName(), responseHeader.getMsgId(), responseHeader.getQueueId(), responseHeader.getQueueOffset()));
    ...
    public void sendHearbeat(final String addr, final HeartbeatData heartbeatData, final long timeoutMillis) throws RemotingException, MQBrokerException, InterruptedException {
        if (!UtilAll.isBlank(projectGroupPrefix)) {
    ...
    public void updateTopicLeval(final String addr, final String topic, String group, final int level, final long timeoutMillis)
            throws RemotingException, MQClientException, InterruptedException {
        UpdateTopicLevelRequestHeader requestHeader = new UpdateTopicLevelRequestHeader();
        requestHeader.setTopic(topic);
        requestHeader.setGroup(group);
        requestHeader.setLevel(level);
        RemotingCommand request =
                RemotingCommand.createRequestCommand(RequestCode.UPDATE_TOPIC_LEVEL, requestHeader);

        RemotingCommand response = this.remotingClient.invokeSync(addr, request, timeoutMillis);
        assert response != null;
        log.info("---------- updateTopicLeval responseCode = {}", response.getCode());
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                return;
            }
