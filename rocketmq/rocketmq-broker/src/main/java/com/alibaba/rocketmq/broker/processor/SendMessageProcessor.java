...
import com.alibaba.rocketmq.broker.util.QueueUtil;
...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        int queueIdInt = QueueUtil.getRandomQueueId(subscriptionGroupConfig.getRetryQueueNums());
        ...
        if (msgExt.getReconsumeTimes() >= subscriptionGroupConfig.getRetryMaxTimes()//
            ...
            queueIdInt = QueueUtil.getRandomQueueId(DLQ_NUMS_PER_GROUP);
     ...
     *
     * @param ctx
     * @param request        Remoting模块中，服务器与客户端通过传递RemotingCommand来交互
     * @param mqtraceContext broker中的消息格式
     * @param requestHeader
     * @return
     * @throws RemotingCommandException
     */
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx, //
        ...
        // 随机指定一个队列
        if (queueIdInt < 0) {
            queueIdInt = QueueUtil.getRandomQueueId(topicConfig.getWriteQueueNums());
        }
        ...
        PutMessageResult putMessageResult = this.brokerController.getMessageStore().putMessage(msgInner);
        if (putMessageResult != null) {
            boolean isSuccess = storePutMessage(putMessageResult,response,msgInner,requestHeader,responseHeader,queueIdInt,mqtraceContext,ctx,request);
            if(isSuccess){
                return null;
            }
        } else {
     ...
     * @return
     */
    public boolean storePutMessage(PutMessageResult putMessageResult,RemotingCommand response,MessageExtBrokerInner msgInner,
                                SendMessageRequestHeader requestHeader,SendMessageResponseHeader responseHeader,int queueIdInt,
                                SendMessageContext mqtraceContext,final ChannelHandlerContext ctx,final RemotingCommand request){
        ...
        if (sendOK) {
            if(!"rocketMqLogListenerTopic".equals(msgInner.getTopic())) {
                log.info("broker SendMessageProcessor sendMessage , topic = " + msgInner.getTopic() + " , msgId = " + putMessageResult.getAppendMessageResult().getMsgId() + " , queueId = " + queueIdInt + " , offset = " + putMessageResult.getAppendMessageResult().getLogicsOffset());
//                log.info(MonitorUtil.log("sendMessage", MonitorUtil.getKeys(), putMessageResult.getAppendMessageResult().getMsgId(), msgInner.getTopic(), this.brokerController.getBrokerConfig().getBrokerName(), this.brokerController.getBrokerConfig().getBrokerIP1()));
            }
            request.addExtField("sendMsgId",putMessageResult.getAppendMessageResult().getMsgId());
            updateBrokerStat(msgInner, putMessageResult, responseHeader, requestHeader, queueIdInt, mqtraceContext, ctx, request, response);
            //save message redis
            updateRedisStat(putMessageResult.getAppendMessageResult().getMsgId(), msgInner.getTopic(), new String(request.getBody()), queueIdInt, putMessageResult.getAppendMessageResult().getLogicsOffset(),msgInner.getKeys(),msgInner.getTags(),msgInner);
            //mq-monitor
            brokerStoreComponent.processMessageObserverLog(msgInner.getTopic(),new Date().getTime(),putMessageResult.getAppendMessageResult().getMsgId(),this.brokerController.getBrokerConfig().getBrokerName());
            return true;
        }
        return false;
    }
    ...
    public void updateRedisStat(String msgId,String topic,String msgContent,Integer queueId,long offset,String keys,String tags,MessageExtBrokerInner msgInner){
        try{
            if(StringUtils.isBlank(msgId)||StringUtils.isBlank(msgContent)||StringUtils.isBlank(topic)){
                log.error("updateRedisStat params is null! msgId = {}",msgId);
                return;
            }
            if(!"resendTask".equals(keys)) {
                brokerStoreComponent.storeMessage(topic, msgId, msgContent,msgInner);
            }else{
                if(StringUtils.isNotBlank(tags)){
                    brokerStoreComponent.removeMessage(topic, tags);
                    brokerStoreComponent.storeMessage(topic, msgId, msgContent,msgInner);
                    log.info("resendTask store new msgId = {}, keys = {}", msgId,keys);
                }
            }
            ...
            log.info("updateRedisStat BROKER_MESSAGE_OFFSET_KET = {} , keys = {}",offsetKey,keys);
            brokerStoreComponent.set(offsetKey,msgId);

            /**
             *  TODO: JUST FOR TEST
             */
//            Set<String> list = brokerStoreComponent.getMessages(topic);
//            log.info("updateRedisStat getMessages topic = {} , list = {}",topic,list);
        }catch (Exception e){
            log.error("updateRedisStat error! msgId = "+msgId,e);
        }
    }
     ...
     * @param response
     */
    public void updateBrokerStat(MessageExtBrokerInner msgInner, PutMessageResult putMessageResult, SendMessageResponseHeader responseHeader, SendMessageRequestHeader requestHeader, int queueIdInt, SendMessageContext mqtraceContext, ChannelHandlerContext ctx, RemotingCommand request, final RemotingCommand response) {
        // 统计
