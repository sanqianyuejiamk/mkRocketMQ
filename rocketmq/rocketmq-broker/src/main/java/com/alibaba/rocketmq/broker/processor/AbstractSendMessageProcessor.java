...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    protected void doResponse(ChannelHandlerContext ctx, RemotingCommand request, final RemotingCommand response) {
        if (!request.isOnewayRPC()) {
            try {
//                log.info("-----[broker D]----- send message remote ip = {} , msgId = {}", ctx.channel().remoteAddress(),request.getExtFields()!=null?request.getExtFields().get("sendMsgId"):"", request.getExtFields()!=null?request.getExtFields():"");
