...
public class AdminBrokerProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        switch (request.getCode()) {
            ...
            // update消息发送等级
            case RequestCode.UPDATE_TOPIC_LEVEL:
                return this.updateTopicLevel(ctx,request);
            // 获取Topic配置
...
//            log.info("------[broker B]---- send message remote ip = {}", clientChannelInfo.getChannel().remoteAddress());
    ...
    private RemotingCommand consumeMessageDirectly(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        log.info("receive consumer client result!!! id = "+requestHeader.getMsgId());
    ...
    private RemotingCommand updateTopicLevel(ChannelHandlerContext ctx, RemotingCommand request)throws RemotingCommandException{
        final RemotingCommand response = RemotingCommand.createResponseCommand(null);
        UpdateTopicLevelRequestHeader requestHeader =
                (UpdateTopicLevelRequestHeader) request.decodeCommandCustomHeader(UpdateTopicLevelRequestHeader.class);

        log.info("updateTopicLevel called by {}", RemotingHelper.parseChannelRemoteAddr(ctx.channel()));

        //update消息的发送等级
        this.brokerController.updateTopicLevel(requestHeader.getTopic(),requestHeader.getGroup(),requestHeader.getLevel());
