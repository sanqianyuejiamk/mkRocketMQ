            ...
            public void run() {
                try {
                    ...
                    if (response != null) {
                        ...
                        try {
//                            log.info("-----[broker G]----- , send message remote ip = {} , msgId = {} , ExtFields = {}", channel.remoteAddress(),request.getExtFields()!=null?request.getExtFields().get("sendMsgId"):"", request.getExtFields()!=null?request.getExtFields():"");
        ...
        if (getMessageResult != null) {
            ...
            case ResponseCode.SUCCESS:
                ...
                this.brokerController.getBrokerStatsManager().incBrokerGetNums(
                        getMessageResult.getMessageCount());
                //consumer ip
                Map<String, Long> messageIds_t = updateMessageIdAndIp(RemotingHelper.parseChannelRemoteAddr(channel), requestHeader, getMessageResult);
                ...
                try {
                    ...
                    log.info("-----[broker H]----- send message remote ip = {} , msgId = {} , ExtFields = {}", channel.remoteAddress(),messageIds_t, response.getExtFields()!=null?response.getExtFields():"");
     ...
     */
    private Map<String, Long> updateMessageIdAndIp(String ip, final PullMessageRequestHeader requestHeader, final GetMessageResult getMessageResult){
        final SocketAddress storeHost =
                new InetSocketAddress(brokerController.getBrokerConfig().getBrokerIP1(),
                        brokerController.getNettyServerConfig().getListenPort());
        Map<String, Long> messageIds =
                this.brokerController.getMessageStore().getMessageIds(requestHeader.getTopic(),
                        ...
                        requestHeader.getQueueOffset() + getMessageResult.getMessageCount(),
                        storeHost);
        for(String msgId:messageIds.keySet()){
            String key = String.format(BrokerStoreComponent.BROKER_MESSAGE_CONSUMER_IP,msgId);
            log.info("updateMessageIdAndIp key = {} , ip = {}",key,ip);
            brokerStoreComponent.setObject(key,0,ip);
        }
        return messageIds;
    }
